...
<html lang="en">
  ...
  <body>
    ...
    <script>
      var FONT_OPTIONS_CLIENT = ${JSON.stringify(FONT_OPTIONS)};
      function escAttrClient(s) {
        return String(s).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;');
      }
      function buildFontOptions(currentValue) {
        var cv = (currentValue || '').trim();
        var html = '';
        var matched = false;
        for (var i = 0; i < FONT_OPTIONS_CLIENT.length; i++) {
          var f = FONT_OPTIONS_CLIENT[i];
          var sel = (cv === f.value || (!cv && !f.value)) ? ' selected' : '';
          if (sel) matched = true;
          html += '<option value="' + escAttrClient(f.value) + '"' + sel + '>' + f.label + (f.google ? ' *' : '') + '</option>';
        }
        if (!matched && cv) {
          html += '<option value="' + escAttrClient(cv) + '" selected>' + escAttrClient(cv) + ' (custom)</option>';
        }
        return html;
      }
