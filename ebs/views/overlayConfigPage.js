  ...
  <body>
      ...
      <div class="panel preview">
        ...
        <div class="${sectionClass("sounds")}" data-section="sounds">
          ...
          <div class="section-body" ${sectionBodyAttr("sounds")}>
            <p style="opacity:0.85; max-width:640px">
              Viewers can spend Bits to trigger sound alerts on your stream.
              Upload sounds, configure settings, and add the OBS Browser Source URL to play alerts on stream.
            </p>

            <!-- Sound Settings -->
            <div style="margin-top:14px">
              <div style="font-weight:600; font-size:14px; margin-bottom:8px;">Settings</div>
            ...
            <!-- Sound List -->
            <div style="margin-top:18px">
              <div style="font-weight:600; font-size:14px; margin-bottom:8px;">Sounds (<span id="soundCount">0</span>/20)</div>
    ...
    <script>
      ...
      function bind() {
        ...
        // devSoundSelect is populated by fetchSoundsAdmin() below
        if (devTestSoundBtn) {
        ...
        const soundGlobalVolumeEl = document.getElementById('soundGlobalVolume');
        const soundGlobalVolumeValEl = document.getElementById('soundGlobalVolumeVal');
        const soundGlobalCooldownEl = document.getElementById('soundGlobalCooldown');
        const soundMaxQueueEl = document.getElementById('soundMaxQueue');
        const saveSoundSettingsBtn = document.getElementById('saveSoundSettings');
        const soundSettingsHintEl = document.getElementById('soundSettingsHint');
        const soundUploadForm = document.getElementById('soundUploadForm');
        const soundFileEl = document.getElementById('soundFile');
        const soundNameEl = document.getElementById('soundName');
        const soundTierEl = document.getElementById('soundTier');
        const soundUploadVolumeEl = document.getElementById('soundUploadVolume');
        const soundUploadVolumeValEl = document.getElementById('soundUploadVolumeVal');
        const soundUploadBtn = document.getElementById('soundUploadBtn');
        const soundUploadHintEl = document.getElementById('soundUploadHint');
        ...
        async function fetchSoundsAdmin() {
          try {
            ...
            populateSoundSelect(soundsCache);
          } catch (err) {
            if (soundListEl) soundListEl.innerHTML = '';
            var hint = document.createElement('div');
            hint.className = 'hint';
            hint.textContent = 'Failed to load sounds';
            if (soundListEl) soundListEl.appendChild(hint);
          }
        }
        ...
        function renderSoundList(sounds) {
          ...
          sounds.forEach(function(s) {
            ...
            delBtn.style.cssText = 'font-size:12px; padding:3px 8px; background:var(--danger,#c0392b); color:#fff; border:none; border-radius:4px; cursor:pointer;';
